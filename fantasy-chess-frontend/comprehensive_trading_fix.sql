...
-- Step 3: Drop and recreate functions
...
DROP FUNCTION IF EXISTS cancel_trade(UUID, UUID);
DROP FUNCTION IF EXISTS mark_notification_seen(UUID);

-- Step 4: Create create_trade function
...
-- Step 5: Create get_trade_notifications function
...
-- Step 6: Create get_user_trades function
...
-- Step 7: Create accept_trade function
...
-- Step 8: Create cancel_trade function
CREATE FUNCTION cancel_trade(
    p_trade_id UUID,
    p_user_id UUID
)
RETURNS BOOLEAN AS $$
...
-- Step 9: Create mark_notification_seen function
CREATE FUNCTION mark_notification_seen(
...
SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('trades', 'trade_notifications');

SELECT 'Functions created:' as status;
SELECT routine_name FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name LIKE '%trade%';
