-- Verify weekly results processing for 2025-07-22
-- Run this in your Supabase SQL editor to check if points were calculated correctly

-- Set the week date to check
DO $$
DECLARE
    check_week_date DATE := '2025-07-22';
    league_count INTEGER;
    lineup_count INTEGER;
    total_points_calc DECIMAL(10,2);
    avg_points_calc DECIMAL(10,2);
BEGIN
    RAISE NOTICE '=== WEEKLY RESULTS VERIFICATION FOR % ===', check_week_date;
...
-- Show detailed breakdown by league
SELECT 
...
FROM leagues l
LEFT JOIN lineups ln ON l.id = ln.league_id AND ln.week_start_date = '2025-07-22'
WHERE l.start_date <= '2025-07-22' AND l.end_date >= '2025-07-22'
GROUP BY l.id, l.name
ORDER BY total_league_points DESC;

-- Show top performers for the week
SELECT 
    u.username,
    l.name as league_name,
    ln.total_points,
...
WHERE ln.week_start_date = '2025-07-22'
ORDER BY ln.total_points DESC
LIMIT 20;

-- Show lineups with 0 points (potential issues)
SELECT 
    u.username,
    l.name as league_name,
    ln.total_points,
...
-- Check if there are any games data for this week
SELECT 
...
-- Show sample games with points for this week
SELECT 
    white,
    black,
    white_points,
...
-- Check if any users are missing lineups (should be handled by auto-creation)
SELECT 
    u.username,
    l.name as league_name,
    'Missing lineup' as issue
FROM leagues l
JOIN unnest(l.member_ids) AS member_id ON true
JOIN users u ON u.id = member_id
LEFT JOIN lineups ln ON ln.user_id = member_id 
    AND ln.league_id = l.id 
    AND ln.week_start_date = '2025-07-22'
WHERE l.start_date <= '2025-07-22' 
  AND l.end_date >= '2025-07-22'
  AND ln.id IS NULL
ORDER BY l.name, u.username;
...
SELECT 
    ...
    COALESCE(AVG(total_points), 0) as avg_points
FROM lineups 
WHERE week_start_date = '2025-07-22' AND total_points > 0
UNION ALL
SELECT 
