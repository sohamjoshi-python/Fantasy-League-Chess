...
-- This creates the function needed for the player history page
-- UPDATED to use the correct 'games' table structure

-- First, drop the old function if it exists
DROP FUNCTION IF EXISTS get_player_weekly_performance(UUID);

-- Function to get player's weekly performance history (using games table with player names)
...
-- Test the function (replace the UUID with an actual player ID from your database)
-- SELECT * FROM get_player_weekly_performance('your-player-uuid-here');
