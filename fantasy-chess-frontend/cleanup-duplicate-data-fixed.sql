-- Clean up duplicate data and fix database issues (Fixed for UUID types)
...
-- 2. Clean up duplicate teams - keep only the one with the latest created_at
DELETE FROM teams 
WHERE id NOT IN (
  SELECT DISTINCT ON (user_id, league_id) id
  FROM teams 
  ORDER BY user_id, league_id, created_at DESC
);

-- 3. Clean up duplicate lineups - keep only the one with the latest created_at
DELETE FROM lineups 
WHERE id NOT IN (
  SELECT DISTINCT ON (user_id, league_id, week_start_date) id
  FROM lineups 
  ORDER BY user_id, league_id, week_start_date, created_at DESC
);

-- 4. Clean up duplicate coin balances - keep only the one with the latest created_at
...
-- 5. Add unique constraints to prevent future duplicates (drop if they exist first)
DO $$ 
BEGIN
    -- Drop constraints if they exist
    ALTER TABLE teams DROP CONSTRAINT IF EXISTS teams_user_league_unique;
    ALTER TABLE lineups DROP CONSTRAINT IF EXISTS lineups_user_league_week_unique;
    ALTER TABLE league_coin_balances DROP CONSTRAINT IF EXISTS league_coin_balances_user_league_unique;
