-- Comprehensive fix for process_weekly_results function
-- Drop all existing versions to avoid conflicts
DROP FUNCTION IF EXISTS public.process_weekly_results(DATE);
DROP FUNCTION IF EXISTS public.process_weekly_results_enhanced(DATE);

-- Create the fixed process_weekly_results function
        ...
        LOOP
            ...
            IF lineup_record IS NULL THEN
                ...
                IF team.player_ids IS NOT NULL AND array_length(team.player_ids, 1) >= 5 THEN
                    ...
                    VALUES (member_id, league_record.id, week_date, auto_lineup_players, 0, NOW(), NOW());
            ...
            IF lineup_record IS NOT NULL THEN
                ...
                RAISE NOTICE 'Total points for user % on % (formatted as %): %', 
                    member_id, week_date, formatted_date, calculated_points;
