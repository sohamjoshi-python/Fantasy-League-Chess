...
SELECT 
  ...
  CASE 
    WHEN up.player_elo >= 3400 THEN 50 -- Elite tier
    WHEN up.player_elo >= 3300 THEN 40 -- Strong tier  
    WHEN up.player_elo >= 3200N30  -- Good tier
    WHEN up.player_elo >= 3100 THEN 20  -- Average tier
    WHEN up.player_elo >= 3000 THEN15-- Developing tier
...
WHERE purchase_price = 0AND bot_id IS NULL;

-- Step 3: Reset all users coin balances to starting amount (50 coins)
