-- Simplified weekly results function that checks for teams instead of draft flags
...
BEGIN
    ...
    -- Loop through all active leagues that have teams (indicating draft is complete)
    FOR league_record IN 
        SELECT l.id, l.name, l.start_date, l.end_date, l.member_ids
        FROM public.leagues l
        WHERE l.start_date <= week_date 
          AND l.end_date >= week_date
          AND EXISTS (
              -- Check if there are teams for this league
              SELECT 1 FROM public.teams t WHERE t.league_id = l.id
          )
    LOOP
        RAISE NOTICE 'Processing league: %', league_record.name;
        
        -- Store member_ids in a variable to avoid any potential issues
        league_member_ids := league_record.member_ids;
        
        -- Check if we have any members
        IF league_member_ids IS NULL OR array_length(league_member_ids, 1) = 0 THEN
            RAISE NOTICE 'League % has no members, skipping', league_record.name;
            CONTINUE;
        END IF;
        
        -- Loop through all users in the league
        FOR user_record IN 
            SELECT unnest(league_member_ids) as user_id
        LOOP
            ...
            IF lineup_record IS NOT NULL THEN
                ...
                IF player_record IS NOT NULL THEN
                    ...
                    -- Look for games throughout the entire week, not just on Monday
                    FOR game_record IN 
                        SELECT g.*, cp.name as player_name
                        FROM public.games g
                        JOIN public.chess_players cp ON g.white = cp.name OR g.black = cp.name
                        WHERE g.date::date >= week_date 
                          AND g.date::date <= week_end_date
                    ...
                    LOOP
                        ...
                        RAISE NOTICE 'Player % scored % points in game % vs %', 
                            game_record.player_name, player_points, game_record.white, game_record.black;
                    END LOOP;
                END IF;
...
GRANT EXECUTE ON FUNCTION public.process_weekly_results_enhanced(DATE) TO authenticated;
