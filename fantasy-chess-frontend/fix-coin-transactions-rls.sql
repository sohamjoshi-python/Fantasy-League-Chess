-- Fix RLS policies for coin_transactions table
-- This allows the record_marketplace_action function to insert transaction records

-- Drop existing policies for coin_transactions
DROP POLICY IF EXISTS "Users can view their own coin transactions" ON coin_transactions;
DROP POLICY IF EXISTS "Users can view bot coin transactions" ON coin_transactions;
DROP POLICY IF EXISTS "System can create transactions" ON coin_transactions;

-- Create new comprehensive policies for coin_transactions
...
CREATE POLICY "System can update transactions" ON coin_transactions
    FOR UPDATE USING (true);

-- Also ensure the record_marketplace_action function has SECURITY DEFINER
-- This allows it to bypass RLS when inserting transaction records
