...
export async function notifyLeagueLifecycle(
  ...
  const events: Array<'marketplace_started' | 'league_started'> =
    event === 'scan' ? ['marketplace_started', 'league_started'] : [event];

  try {
    for (const lifecycleEvent of events) {
      const { data: recipients, error } = await supabase.rpc('claim_league_lifecycle_emails', {
        p_event: lifecycleEvent,
        p_league_id: leagueId,
      });

      if (error) {
        console.error('League lifecycle email claim failed:', error);
        continue;
      }

      for (const recipient of recipients || []) {
        if (!recipient.email) continue;
        const { subject, htmlContent, textContent } = createLeagueLifecycleEmail({
          recipientName: recipient.username || recipient.email.split('@')[0] || 'there',
          leagueName: recipient.league_name || 'your league',
          leagueId,
          startDate: recipient.start_date,
        ...
        const { error: emailError } = await supabase.functions.invoke('send-resend-email', {
          body: {
            ...
            leagueId,
            metadata: { source: 'league_lifecycle', event: lifecycleEvent },
          },
        });

        if (emailError) {
          console.error('League lifecycle email send failed:', emailError);
        }
      }
    }
  } catch (error) {
...
function formatLeagueEmailDate(ymd: string | null): string {
...
function createLeagueLifecycleEmail({
  recipientName,
  leagueName,
  leagueId,
  startDate,
  ...
  const leagueUrl = `https://fantasyleaguechess.com/league/${leagueId}`;
  const startDateLabel = formatLeagueEmailDate(startDate);
