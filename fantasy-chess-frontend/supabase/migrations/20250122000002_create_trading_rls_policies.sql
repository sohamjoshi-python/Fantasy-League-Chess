-- Create RLS Policies for Trading System
-- This migration adds Row Level Security policies for trades and trade_notifications tables
...
-- INSERT policy: System can create notifications for league members (except seller)
-- This is typically done by database functions, so we allow authenticated users
-- to insert notifications for users in leagues they're members of
...
-- These functions need to bypass RLS for system operations

-- Grant execute permissions on trading functions
...
COMMENT ON POLICY "trades_select_policy" ON trades IS 'Allows league members to view all trades in their leagues';
COMMENT ON POLICY "trades_insert_policy" ON trades IS 'Allows users to create trades for players they own in leagues they belong to';
COMMENT ON POLICY "trades_update_policy" ON trades IS 'Allows sellers to modify their trades and buyers to accept trades';
COMMENT ON POLICY "trades_delete_policy" ON trades IS 'Allows users to delete trades they created';

COMMENT ON POLICY "trade_notifications_select_policy" ON trade_notifications IS 'Allows users to view their own trade notifications';
COMMENT ON POLICY "trade_notifications_insert_policy" ON trade_notifications IS 'Allows creation of notifications for league members';
COMMENT ON POLICY "trade_notifications_update_policy" ON trade_notifications IS 'Allows users to mark their notifications as seen';
COMMENT ON POLICY "trade_notifications_delete_policy" ON trade_notifications IS 'Allows users to delete their own notifications';
