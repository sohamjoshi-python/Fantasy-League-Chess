-- Fix league discovery/join around local calendar dates and UUID[] inserts.
-- The frontend treats league start_date as an app-local calendar date, not UTC.
...
BEGIN
  ...
  week_start := app_today - (((EXTRACT(DOW FROM app_today)::int + 6) % 7)::int);

  INSERT INTO public.teams (user_id, league_id, player_ids)
  VALUES (user_id_input, league_id_input, ARRAY[]::UUID[])
  ON CONFLICT (user_id, league_id) DO NOTHING;

  INSERT INTO public.league_coin_balances (user_id, league_id, coin_balance)
  VALUES (user_id_input, league_id_input, 50)
  ON CONFLICT (user_id, league_id) DO NOTHING;

  INSERT INTO public.lineups (user_id, league_id, week_start_date, player_ids, total_points)
  VALUES (user_id_input, league_id_input, week_start, ARRAY[]::UUID[], 0.00)
...
GRANT EXECUTE ON FUNCTION public.create_league_data_for_user(UUID, UUID) TO authenticated;
