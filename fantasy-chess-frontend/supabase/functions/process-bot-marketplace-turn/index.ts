...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Bot team has ${team.player_ids?.length || 0}/10 players`)
    ...
    if (playersError) {
      console.error("Failed to fetch available players:", playersError)
      return new Response(
        JSON.stringify({ success: false, error: "Failed to fetch available players" }),
        { 
          status: 500, 
          headers: { ...corsHeaders, "Content-Type": "application/json" } 
        }
    ...
    if (selectedPlayer && selectedPrice > 0) {
      ...
      return new Response(
        JSON.stringify({ success: true, action: "skip", reason: "Cannot afford any players" }),
        { 
          status: 200, 
          headers: { ...corsHeaders, "Content-Type": "application/json" } 
        }
