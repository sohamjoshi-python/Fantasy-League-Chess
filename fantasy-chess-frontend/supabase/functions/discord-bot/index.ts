...
// Create a new Discord channel for a league
async function createLeagueDiscordChannel(leagueName: string, leagueId: string): Promise<{ channelId: string, inviteUrl: string }> {
  try {
    ...
    // Create the Discord channel in the main server
    const channelData = await discordApiRequest('/channels', {
      method: 'POST',
      body: JSON.stringify({
        guild_id: DISCORD_MAIN_SERVER_ID,
        name: `🏆-${leagueName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
        type: 0, // Text channel
        topic: `League channel for ${leagueName} - Strategy, announcements, and community discussion`,
