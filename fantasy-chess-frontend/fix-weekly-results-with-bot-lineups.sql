-- Fix process_weekly_results function to include bot lineup creation
...
BEGIN
    ...
    LOOP
        ...
        -- Format the date for game matching - use YYYY.MM.DD format to match games table
        ...
        -- Process bot if it exists for this league
        SELECT * INTO bot_record
        FROM public.bots
        WHERE league_id = league_record.id;

        IF bot_record IS NOT NULL THEN
            RAISE NOTICE 'Processing bot % for league %', bot_record.name, league_record.name;

            -- Get bot's lineup for the week
            SELECT * INTO lineup_record 
            FROM public.lineups 
            ...
            -- If no lineup exists, create one automatically with best players
            IF lineup_record IS NULL THEN
                -- Get bot's team
                SELECT * INTO team
                FROM public.teams
                ...
                IF team.player_ids IS NOT NULL AND array_length(team.player_ids, 1) >= 5 THEN
                    ...
                    VALUES (bot_record.id, league_record.id, league_week_date, auto_lineup_players, 0, NOW(), NOW());
            ...
            IF lineup_record IS NOT NULL THEN
                ...
                -- Debug: Show which players are being processed
                RAISE NOTICE 'Processing lineup for bot %: player_ids %, player_names %', bot_record.name, player_ids, player_names;
                ...
                -- Debug: Show the total points calculated
                RAISE NOTICE 'Total points for bot % on % (formatted as %): %', 
                    bot_record.name, league_week_date, formatted_date, calculated_points;
