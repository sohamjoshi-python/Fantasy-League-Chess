...
-- Add Discord fields to leagues table
ALTER TABLE leagues 
ADD COLUMN IF NOT EXISTS discord_server_id TEXT,
ADD COLUMN IF NOT EXISTS discord_invite_link TEXT;

-- Add Discord user ID to users table (optional for future Discord account linking)
ALTER TABLE users 
ADD COLUMN IF NOT EXISTS discord_user_id TEXT;

-- Create index for Discord server lookups
CREATE INDEX IF NOT EXISTS idx_leagues_discord_server_id ON leagues(discord_server_id);

-- Create a function to update Discord server info when league is created
...
-- Create a view for Discord server information
...
-- Create a function to get Discord info for a league
...
-- Optional: Create a table to track Discord announcements
...
-- Create indexes for Discord announcements
...
-- Grant permissions on discord_announcements table
GRANT SELECT, INSERT, UPDATE ON discord_announcements TO authenticated;

-- Create a function to log Discord announcements
